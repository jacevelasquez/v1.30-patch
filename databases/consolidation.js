...
function JSON2Sqlite(json,name,form_name,username){
   var col = [];
   var values =[];
   var  sql ="";
   var  rn = 0;
...
exports.upload =  function (req, res, next) {
  ...
  fs.appendFile('logs_'+req.session.username+'.txt', '['+dateLog+'] Consolidating '+req.file.originalname+' \n');
  ...
  csv_upload(name,form_name,username);
...
function csv_upload(name,form_name,username){
csv()
.fromFile(csvFilePath)
.on('json',(jsonObj)=>{
    JSON2Sqlite(jsonObj,name,form_name,username);
})
.on('done',(error)=>{
