...
db2.all("SELECT * FROM f11 WHERE eacode = ? and  hcn= ? and shsn= ? and MEMBER_CODE=?",[eacode2, hcn2, shsn2, MEMBER_CODE2], function(err,rows){
	...
	db2.all("SELECT * FROM f411 WHERE eacode = ? and  hcn= ? and shsn= ? and MEMBER_CODE=?",[eacode2, hcn2, shsn2, MEMBER_CODE2], function(err,rows2){
		...
		db2.all("SELECT * FROM questions WHERE qkey LIKE '%f411_%' order by qkey ASC", function(err,rows3){
			...
			db2.all("SELECT * FROM answers WHERE qkey LIKE '%f411_%' order by variable_name ASC", function(err,rows4){
				
				db2.all("SELECT * FROM survey_forms WHERE table_name='f411'", function(err,rows5){

					res.render('form411.ejs',{array:rows,array2:rows2,array3:rows3,array4:rows4,array5:rows5,layout:false,session:req.session}); 
				});
			}); 
		});	
	});	
...
exports.post = function(req, res){
	...
	db2.all("UPDATE f411 SET "+set.replace(/,\s*$/, '')+", INTERVIEW_TIME =?,DATE_EDIT=? WHERE eacode = ? and hcn = ? and shsn = ? and MEMBER_CODE = ?",arr,function(err,rows){
		if (err) {
...
exports.time = function(req, res){
	...
	db2.all("UPDATE f411 SET TIME_START='"+req.body.TIME_START+"', TIME_END='"+req.body.TIME_END+"' WHERE eacode = '"+req.body.eacode+"' and hcn = '"+req.body.hcn+"' and shsn = '"+req.body.shsn+"' and MEMBER_CODE = '"+req.body.MEMBER_CODE+"'",function(err,rows){
		if (err) {
              console.log("Error Updating : %s ",err );
		}
...
exports.skipping = function(req, res){
	var obj = {};
	  db2.all("SELECT * FROM skipping WHERE answer_qkey like '%f411%' ORDER BY answer_qkey ASC", function(err,rows){
		if (err) {
              console.log("Error Selecting : %s ",err );
		}
