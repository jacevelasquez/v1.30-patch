...
db2.all("SELECT * FROM f12 WHERE eacode = ? and  hcn= ? and shsn= ?",[eacode2, hcn2, shsn2], function(err,rows){

    db2.all("SELECT * FROM questions WHERE qkey LIKE '%f12_%' ORDER BY qkey ASC", function(err,rows3){
		
		db2.all("SELECT * FROM answers WHERE qkey LIKE '%f12_%' order by variable_name ASC", function(err,rows4){
...
exports.post = function(req, res){
	...
	db2.all("UPDATE f12 SET "+set.replace(/,\s*$/, '')+", INTERVIEW_TIME =?,DATE_EDIT=? WHERE eacode = ? and hcn = ? and shsn = ?",arr,function(err,rows){
		if (err) {
...
exports.time = function(req, res){
	...
	db2.all("UPDATE f12 SET TIME_START='"+req.body.TIME_START+"', TIME_END='"+req.body.TIME_END+"' WHERE eacode = '"+req.body.eacode+"' and hcn = '"+req.body.hcn+"' and shsn = '"+req.body.shsn+"'",function(err,rows){
		if (err) {
              console.log("Error Updating : %s ",err );
		}
...
exports.skipping = function(req, res){
	var obj = {};
	  db2.all("SELECT * FROM skipping WHERE answer_qkey like '%f12%' ORDER BY answer_qkey ASC", function(err,rows){
