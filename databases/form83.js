...
db2.all("SELECT * FROM f11 WHERE eacode = ? and  hcn= ? and shsn= ? and MEMBER_CODE=?",[eacode2, hcn2, shsn2, MEMBER_CODE2], function(err,rows){
	...
	db2.all("SELECT * FROM f83 WHERE eacode = ? and  hcn= ? and shsn= ? and MEMBER_CODE=?",[eacode2, hcn2, shsn2, MEMBER_CODE2], function(err,rows2){
	 
		db2.all("SELECT * FROM questions WHERE qkey LIKE '%f83_%' order by qkey ASC", function(err,rows3){

			db2.all("SELECT * FROM answers WHERE qkey LIKE '%f83_%' order by variable_name ASC", function(err,rows4){
				
				db2.all("SELECT * FROM survey_forms WHERE table_name='f83'", function(err,rows5){

					res.render('form83.ejs',{array:rows,array2:rows2,array3:rows3,array4:rows4,array5:rows5,layout:false,session:req.session}); 
				});
			}); 
		});	
	});	
...
exports.post = function(req, res){
	...
	db2.all("UPDATE f83 SET "+set.replace(/,\s*$/, '')+", INTERVIEW_TIME =?,DATE_EDIT=? WHERE eacode = ? and hcn = ? and shsn = ? and MEMBER_CODE = ?",arr,function(err,rows){
		if (err) {
              console.log("Error Updating : %s ",err );
		}
		else console.log("Success updating form 8.3");
	})
...
exports.view = function(req, res){
	...
	var MEMBER_CODE = req.body.MEMBER_CODE;
	  db2.all("SELECT * FROM f83 WHERE eacode = ? and  hcn= ? and shsn= ? and MEMBER_CODE=?",[eacode, hcn, shsn, MEMBER_CODE], function(err,rows2){
		  if (err) {
...
exports.skipping = function(req, res){
	var obj = {};
	  db2.all("SELECT * FROM skipping WHERE answer_qkey like '%f83%' ORDER BY answer_qkey ASC", function(err,rows){
		if (err) {
              console.log("Error Selecting : %s ",err );
		}
